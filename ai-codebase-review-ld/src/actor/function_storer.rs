...
pub async fn run(context: SteadyContext
        ,archived_function_rx: SteadyRx<ArchivedFunction>, state: SteadyState<FunctionstorerInternalState>
  ...
  let cmd =  into_monitor!(context, [archived_function_rx],[]);
  internal_behavior(cmd,archived_function_rx, state).await
}

async fn internal_behavior<C: SteadyCommander>(mut cmd: C,archived_function_rx: SteadyRx<ArchivedFunction>, state: SteadyState<FunctionstorerInternalState>
 ) -> Result<(),Box<dyn Error>> {
   ...
   while cmd.is_running(&mut ||archived_function_rx.is_closed_and_empty()) {
     ...
     // returned early due to a shutdown request or closed channel.
         let clean = await_for_all!(cmd.wait_closed_or_avail_units(&mut archived_function_rx,1)    );
...
pub async fn run(context: SteadyContext
    ...
    let mut cmd =  into_monitor!(context, [archived_function_rx],[]);
    if let Some(responder) = cmd.sidechannel_responder() {
         let mut archived_function_rx = archived_function_rx.lock().await;
         while cmd.is_running(&mut ||
             archived_function_rx.is_closed_and_empty()) {
                // in main use graph.sidechannel_director node_call(msg,"FunctionStorer")
                let _did_check = responder.equals_responder(&mut cmd,&mut archived_function_rx).await;
         }
    }
    Ok(())
}
    ...
    pub(crate) async fn test_simple_process() {
       ...
       graph.actor_builder()
                    ...
                    .build_spawn( move |context|
                            internal_behavior(context, archived_function_rx.clone(), state.clone())
                     );
