...
async fn internal_behavior<C: SteadyCommander>(mut cmd: C,file_list_rx: SteadyRx<FileMetadata>,file_content_tx: SteadyTx<FileContent>, state: SteadyState<ReadfileInternalState>
 ) -> Result<(),Box<dyn Error>> {
   ...
   while cmd.is_running(&mut ||file_list_rx.is_closed_and_empty() && file_content_tx.mark_closed()) {
     ...
     // returned early due to a shutdown request or closed channel.
         ...
         let mut data = None; 
          ...
          //TODO:  here is an example reading from file_list_rx
          ...
          match cmd.try_take(&mut file_list_rx) {
              Some(rec) => {
                  trace!("got rec: {:?}", rec);
                   ...
                   let file_content = read_file_with_line_numbers(Path::new(&rec.path)).unwrap_or_else(|| {
                    eprintln!("Failed to read file: {}", rec.path);
                    String::new()
                    });
                    data = Some(FileContent { path: rec.path.clone(), content: file_content });
                     if let Some(data) = data {
                    if(file_content_tx.mark_closed())
                    {
                    match cmd.try_send(&mut file_content_tx, data) {
                        Ok(()) => {
